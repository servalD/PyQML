...
class MatplotlibIconProvider(QtQuick.QQuickImageProvider):
    ...
    def requestImage(self, _id, size):
        img = QtGui.QImage(os.path.join(self.basedir, _id + '.png'))
        size = img.size()
        return img, size
        
    def requestPixmap(self, _id, size):
        img, size = self.requestImage(_id, size)
...
class FigureCanvasQtQuickAgg(QtQuick.QQuickPaintedItem, FigureCanvasAgg):
    ...
    def drawRectangle(self, rect):
        self._drawRect = rect
        self.update()
    def switshChannels(self, imArray, ind1, ind2):
        """
        Arguments:
            imArray {numpy.array} -- Image channels
            ind1 {int} -- channel index one to switch with the two
            ind2 {int} -- channel index two to switch with the one
        """
        # testPos = 267
        # initData = imArray[testPos,testPos].copy()
        tmp = imArray[:,:,ind1].copy()
        imArray[:,:,ind1] = imArray[:,:,ind2]
        imArray[:,:,ind2] = tmp
        # print("switsh channels init: ", initData, "result: ", imArray[testPos,testPos])

    def popChannelTo(self, imArray, chanInd, newInd):
        """
        Arguments:
            imArray {numpy.array} -- Image channels
            ind1 {int} -- channel index one to switch with the two
        ...
        """
        # testPos = 267
        # initData = imArray[100,90].copy()
        if chanInd!=newInd:
            tmp = imArray[:,:,chanInd].copy()
            if chanInd<newInd:
                imArray[:,:,chanInd:newInd] = imArray[:,:,chanInd+1:newInd+1]
            elif chanInd>newInd:
                imArray[:,:,newInd+1:chanInd+1] = imArray[:,:,newInd:chanInd]
            imArray[:,:,newInd] = tmp
        # print("pop channels to init: ", initData, "result: ", imArray[100,90])
    ...
    def paint(self, p):
        ...
        if self.blitbox is None:
            ...
            # (bgra).
            imBuffer = np.frombuffer(self.renderer._renderer.buffer_rgba(), np.int8).reshape((int(self.renderer.width),int(self.renderer.height), 4)).copy()
            # it's suposed to be a rgba format but in Qt, channels are not intuitive for me, so i convert it to ABGR32
            self.switshChannels(imBuffer, 0, 2)
            # self.popChannelTo(imBuffer, 3, 0)
            # self.switshChannels(imBuffer, 1, 3)
            # convert the Agg rendered image -> qImage
            
            qImage = QtGui.QImage(imBuffer, self.renderer.width,
                                self.renderer.height,
                                QtGui.QImage.Format_ARGB32)
            if not os.path.isfile("mplRenderer.png"):
                qImage.save("mplRenderer.png")
